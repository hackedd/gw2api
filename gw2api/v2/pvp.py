...
class PvpSeasonEndpoint(LocaleAwareEndpoint):
    ...
    def get_leaderboard_endpoint(self, season_id, board):
        name = "%s/%s/leaderboards/%s" % (self.name, season_id, board)
        return PvpSeasonLeaderboardEndpoint(name)

    def get_leaderboard(self, season_id, board, page=0, page_size=None):
        endpoint = self.get_leaderboard_endpoint(season_id, board)
